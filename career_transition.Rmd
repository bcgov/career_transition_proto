...
teer_table2 <- function(noc1, noc2){
  ...
  colnames(temp) <- c(str_trunc(noc1, trunc_at),str_trunc(noc2, trunc_at))
...
wage_plot <- function(){
  wages%>%
    ...
    scale_y_discrete(labels = function(x) str_trunc(x, trunc_at)) + 
...
fos_table <- function(){
  ...
  colnames(fos_table) <- c(str_trunc(input$noc, trunc_at),str_trunc(input$destination, trunc_at))
...
level_plot <- function(levels){
  levels|>
    ...
    scale_fill_manual(values=c("grey85", "grey20"),
                      labels = function(x) str_trunc(x, trunc_at)   
                      )+ 
    labs(x="",
         y="",
         fill="",
...
The occupations &uarr; are the closest in terms of 161 ONET attributes:
...
### **`r renderUI({paste0("Top 5 ", input$category, " for ", str_trunc(input$destination, trunc_at))})`**
