...
teer_table2 <- function(noc1, noc2){
  ...
  colnames(temp) <- c(str_trunc(noc1, input$trunc_at),str_trunc(noc2, input$trunc_at))
  DT::datatable(temp, rownames = FALSE, options = list(dom="t"))|>
    DT::formatStyle(
      columns = names(temp),
      fontSize = paste0(input$font, "px")
    )
}
...
wage_plot <- function(){
  wages%>%
    ...
    scale_y_discrete(labels = function(x) str_trunc(x, input$trunc_at)) + 
...
fos_table <- function(){
  ...
  colnames(fos_table) <- c(str_trunc(input$noc, input$trunc_at),str_trunc(input$destination, input$trunc_at))
  DT::datatable(fos_table, rownames = FALSE, options=list(dom='t'))|>
    DT::formatStyle(
      columns = names(fos_table),
      fontSize = paste0(input$font, "px")
    )
}
...
level_plot <- function(levels){
  levels|>
    ggplot(aes(value, subcategory, fill=noc))+
    geom_col(width = 0.7, position = "dodge")+
    theme_minimal()+
    scale_fill_manual(values=c("grey85", "grey20"),
                      labels = function(x) str_trunc(x, input$trunc_at)   
                      )+ 
    labs(x="",
         y="",
         fill="",
...
min_wage <- round(min(wages$median_wage, na.rm = TRUE))-1
max_wage <- round(max(wages$median_wage,na.rm = TRUE)) +1
...
renderUI({#need the renderUI because the list of choices "nn()" is a reactive object
 # req(length(nn()) > 0, "No matching occupations were found. Please adjust your filters.")
...
The occupations above are the closest in terms of 161 ONET attributes (Skills, Abilities, Knowledge, Work activities)
...
```{r}
checkboxInput("teer", "Disregard lower TEER (i.e. more education) occupations?", FALSE)
checkboxInput("wage", "Disregard occupations that have a lower median wage?", FALSE)
```
...
#reactive elements----------------
user_theme <- reactive({
  theme_minimal(base_size = input$font, base_family = "Arial")
})


#need to replace all the manual themes in the plots....
...
tbbl <- reactive({
  ...
  if(input$wage==TRUE){
    scaled_char <- scaled_char|>
      filter(median_wage>= scaled_char$median_wage[noc==input$noc])
  }
  scaled_char%>%
...
temp <- reactive({
  if(nrow(mat())<10) stop('Filters too strict: no remaining occupations to compare.')
  FNN::get.knnx(mat(), query = q(), k = 10)
})
nn <-  reactive({
  rownames(mat())[as.vector(temp()[["nn.index"]])] #the names of the closest occupations
})
...
### **`r renderUI({paste0("Top 5 ", input$category, " for ", str_trunc(input$destination, input$trunc_at))})`**
...
### **LMO 2025 Projected 10 year Job Openings**
